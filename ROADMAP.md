...
| Cleanup | `bootstrap.sh` line ~111 calls `rvm_env_ensure "${RVM_REPO_DIR}" "$(command -v python3.12 …)"` with a second positional arg; `rvm_env_ensure()` only reads `$1` | ○ | Found while auditing for hangs 2026-09-04, unrelated to any of them — harmless dead arg, but misleading to a future reader who might assume it does something. Minor, low priority |
...
| 2026-09-04 | Root cause found for both worker failures: the 2026-09-03 daemon refactor (`2c836ae`) silently invalidated the env cache for every project by changing `RVM_VENV`'s basename (which feeds `rvm_env_hash`) without bumping `v3`. Fixed by the one-project-per-box revert (`d23fb15`) — not yet validated by a real launch | CLAUDE.md rule 4's exact failure mode, confirmed from the commit's own message ("the venv name joins the cache key") rather than inferred. Every boot since 2026-09-03 has been forcing a full rebuild instead of a ~13s restore; Lora's dependency set has grown across 91 commits since the ~6min build figure was measured, so a much slower real build time is a plausible full explanation for the "hangs" being genuinely slow builds, not stuck processes |
